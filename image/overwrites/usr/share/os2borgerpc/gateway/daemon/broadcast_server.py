...
from os2borgerpc_utils.os2borgerpc_config import get_config
...
class OS2borgerPCDaemon(Daemon):
    def run(self):
        server_ip = None;
        if len(self.args) > 0:
...
if __name__ == "__main__":
    daemon = OS2borgerPCDaemon(
        '/var/run/os2borgerpc-localserver-daemon.pid',
        '/dev/null',
        '/var/log/os2borgerpc_daemon.log',
        '/var/log/os2borgerpc_daemon_err.log'
    )
    if len(sys.argv) == 2:
        if 'start' == sys.argv[1]:
            if len(sys.argv) > 2:
